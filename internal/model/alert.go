...
// Alert представляет сработавший инцидент (алерт) для транспортного средства
// Является денормализованным снапшотом правила на момент срабатывания и хранится в помесячно партиционированной таблице
type Alert struct {
	ID             int           // Уникальный идентификатор алерта
	OrganizationID int           // Идентификатор организации, которой принадлежит машина
	VehicleID      int           // Идентификатор автомобиля
	RuleID         int           // Идентификатор правила, породившего алерт
	...
	Value          *float64      // Фактическое измеренное значение при срабатывании (например, скорость или остаток топлива)
